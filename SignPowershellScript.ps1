...
# To run this for the first time, you'll need to manually:
# Set-ExecutionPolicy ByPass

param(
    [Parameter(Mandatory)] [String]$fileToSign
)

$authenticode = Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Subject -eq "CN=ATA Authenticode"}

#Write-Host "a." + $authenticode.GetExpirationDateString()

# Clear out any old Authenticode certs

if ($authenticode) {
    $most_recent = ($authenticode.GetExpirationDateString() | Measure-Object -Maximum).Maximum
    foreach ($a in $authenticode)
    {
        if ($authenticode.GetExpirationDateString() -ne $most_recent)
        {
            Remove-Item -Path ("Cert:\LocalMachine\My\" + $a.Thumbprint)
        }
    }
}

# Try again to make sure we get just one cert
$authenticode = Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Subject -eq "CN=ATA Authenticode"}

#Write-Host "b." + $authenticode.GetExpirationDateString()

# Get a new Signing cert if we need to
if (-not $authenticode -or [datetime]$authenticode.GetExpirationDateString() -lt (Get-Date) )
{  
    ...
    # Write-Host "c." + $authenticode.GetExpirationDateString()
...
if (-not (Get-ChildItem Cert:\LocalMachine\TrustedPublisher | Where-Object {$_.Subject -eq "CN=ATA Authenticode"}))
{
...
Set-AuthenticodeSignature -FilePath $fileToSign -Certificate $codeCertificate -TimeStampServer http://timestamp.digicert.com

# Restore more secure execution policy
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUTTVKkcOiZN1//dtwYu30oXlZ
# WmigghZEMIIDBjCCAe6gAwIBAgIQfgB16oPhJYdCA8+jPApo0zANBgkqhkiG9w0B
...
# AYI3AgEVMCMGCSqGSIb3DQEJBDEWBBRzhI09LX7cECbHNJy/RP/wwbQ/QzANBgkq
# hkiG9w0BAQEFAASCAQCxxxJGyH9xAHjsDFw5rsDBG49mhW+ZCnkml7pAHX+A1rPX
# TNSLXtorUYmXqkMXSeGk2p2/8RktnFzuKjnJgVamodSuea3FPMgRu6Yq6P0/Bx/D
# 7ZBc8SQvg6KYsdLCl71Ri+JPD9k3HRJau6qkXL6DPZb6oQDSl8+kB4oD6qVmkkkT
# sUREv+cBmXI/Qov0ItHmrtJ6XqsiiS+AwYGbWTy1SMMZ0J72MOUKhIrLnXTefXEi
# tDih6bL8tJRqLo4K0EEGncRKTrlCZ1WJC67ZfTp9sGtVuBPlw5TpG3dxL1arG5Gb
# z/tk7podJik+oHAnKN8Y9G5mgvw+z0Fx2LGKVy+7oYIDJjCCAyIGCSqGSIb3DQEJ
...
# AQkFMQ8XDTI2MDEwMTIyNDgyOVowLwYJKoZIhvcNAQkEMSIEIOj4vm+FYgQRQDSc
# GZ9AK0XdqsiNleII1aDjD/1mnYzjMA0GCSqGSIb3DQEBAQUABIICABovGSEH4odn
# S7hyaDl82/RCn2qZkLBNnS/M4Su1yWVMBI/gS2W4jwHFTYMOyhe0HvU4n+36++fO
# u2aVO97aygEccZzNJ6p6uwIifCh0TBm8RDq1rQHpBJmgVeGOsfyq7OELoDmFAGXt
# LkYIb/CY9UDAl0a+uX4vrYUKiufUmys1OsAS+EbUwD9jaCkAi8OEQ/PTYF6uB4aa
# j+1FxAmvbyurNgPlVg6ASgGpX4svqZHx4RQ1/fi95BPQa4gDXx+M9XYbmzd92unj
# 8OYNfNSF+unp0KLT8iEo9avbU3FZuiz8acSCp/oliqBYwGWvsLUSDn3VJ4jF36JK
# RoC3aJXdgBEGeikbr8aPh2zZdbahl4GHcf4oWZU++Myt7AF0iMh2MBqhAm2t3bWl
# 7s2YlYkssyPJILTRH9mG1Ofu0Ti8s9QFSSi+EWN+UsQLnyitW1CZbNkGe5U8+SJ/
# YaWPSb4KGdtxHVOHpRh+HDMfy58Y/qvXllDS9mcbmEWEHu3ICoOS9SFjoUTT3zLk
# zQlvRdyKWiHCJPn7+F9S15OMnwE35aKpqJqHtn3AxxnwYHUAIGva+eXKTYzBw/eM
# VRoWo0Lr7fiVvyc5Hke5pUewSM8s/OTQR1nkA/AeW+6E52cvNCctvIQ+5djIPXdU
# 3M/IdyQi1OtdslpFUWgmJtxnHXVGzCpz
